...
!~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~
      ...
      REAL*8 Tstart,Tend
      REAL*8 Y(74),AbsTol(74),RelTol(74)
      INTEGER IPAR(20)
      REAL*8 RPAR(20)
      INTEGER IERR
...
!~~~> Output: Error indicator
      INTEGER IERR
! ~~~~ Local variables           
      REAL*8 Ynew(74), Fcn0(74), Fcn(74),
     &       K(74*ros_S), dFdT(74),
     &       Jac0(920), Ghimj(920)
      REAL*8 H, Hnew, HC, HG, Fac, Tau 
      REAL*8 Err, Yerr(74)
      INTEGER Pivot(74), Direction, ioffset, j, istage
...
!~~~>   Compute the stages
	 ...
	 ! Current istage offset. Current istage vector is K(ioffset+1:ioffset+74)
	 ioffset = 74*(istage-1)
	 
	 ! For the 1st istage the function has been computed previously
	 IF ( istage.EQ.1 ) THEN
	   CALL WCOPY(74,Fcn0,1,Fcn,1)
	 ! istage>1 and a new function evaluation is needed at the current istage
	 ELSEIF ( ros_NewF(istage) ) THEN
	   CALL WCOPY(74,Y,1,Ynew,1)
	   DO j = 1, istage-1
	     CALL WAXPY(74,ros_A((istage-1)*(istage-2)/2+j),
     &                   K(74*(j-1)+1),1,Ynew,1) 
	   END DO
	   Tau = T + ros_Alpha(istage)*Direction*H
           CALL ode_Fun(Tau,Ynew,Fcn)
	 END IF ! if istage.EQ.1 elseif ros_NewF(istage)
	 CALL WCOPY(74,Fcn,1,K(ioffset+1),1)
	 DO j = 1, istage-1
	   HC = ros_C((istage-1)*(istage-2)/2+j)/(Direction*H)
	   CALL WAXPY(74,HC,K(74*(j-1)+1),1,K(ioffset+1),1)
	 END DO
         IF ((.NOT. Autonomous).AND.(ros_Gamma(istage).NE.ZERO)) THEN
           HG = Direction*H*ros_Gamma(istage)
	   CALL WAXPY(74,HG,dFdT,1,K(ioffset+1),1)
         END IF
...
!~~~>  Compute the new solution 
      CALL WCOPY(74,Y,1,Ynew,1)
      DO j=1,ros_S
	 CALL WAXPY(74,ros_M(j),K(74*(j-1)+1),1,Ynew,1)
      END DO

!~~~>  Compute the error estimation 
      CALL WSCAL(74,ZERO,Yerr,1)
      DO j=1,ros_S        
	CALL WAXPY(74,ros_E(j),K(74*(j-1)+1),1,Yerr,1)
      END DO 
...
!~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~
      REAL*8 FUNCTION ros_ErrorNorm ( Y, Ynew, Yerr, 
...
!~~~> Input arguments   
      REAL*8 T, Roundoff, Y(74), Fcn0(74) 
      EXTERNAL ode_Fun  
!~~~> Output arguments      
      REAL*8 dFdT(74)   
!~~~> Global variables     
...
!~~~> Local variables     
      ...
      CALL WAXPY(74,(-ONE),Fcn0,1,dFdT,1)
      CALL WSCAL(74,(ONE/Delta),dFdT,1)
...
!~~~>    Construct Ghimj = 1/(H*ham) - Jac0
        CALL WCOPY(920,Jac0,1,Ghimj,1)
        CALL WSCAL(920,(-ONE),Ghimj,1)
...
!        CALL  DGETRF( 74, 74, A, 74, Pivot, ising ) 
...
!~~~> Note: for a full matrix use Lapack:
!        NRHS = 1
!        CALL  DGETRS( 'N', 74 , NRHS, A, 74, Pivot, b, 74, INFO )
