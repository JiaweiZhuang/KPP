...
C                                                                  
C File                 : small_strato_Integrator.f                 
...
!~~~> Output: Error indicator
      INTEGER IERR
! ~~~~ Local variables           
      REAL*8 Ynew(5), Fcn0(5), Fcn(5),
     &       K(5*ros_S), dFdT(5),
     &       Jac0(19), Ghimj(19)
      REAL*8 H, Hnew, HC, HG, Fac, Tau 
      REAL*8 Err, Yerr(5)
      INTEGER Pivot(5), Direction, ioffset, j, istage
...
!~~~>   Compute the stages
	 ...
	 ! Current istage offset. Current istage vector is K(ioffset+1:ioffset+5)
	 ioffset = 5*(istage-1)
	 
	 ! For the 1st istage the function has been computed previously
	 IF ( istage.EQ.1 ) THEN
	   CALL WCOPY(5,Fcn0,1,Fcn,1)
	 ! istage>1 and a new function evaluation is needed at the current istage
	 ELSEIF ( ros_NewF(istage) ) THEN
	   CALL WCOPY(5,Y,1,Ynew,1)
	   DO j = 1, istage-1
	     CALL WAXPY(5,ros_A((istage-1)*(istage-2)/2+j),
     &                   K(5*(j-1)+1),1,Ynew,1) 
	   END DO
	   Tau = T + ros_Alpha(istage)*Direction*H
           CALL ode_Fun(Tau,Ynew,Fcn)
	 END IF ! if istage.EQ.1 elseif ros_NewF(istage)
	 CALL WCOPY(5,Fcn,1,K(ioffset+1),1)
	 DO j = 1, istage-1
	   HC = ros_C((istage-1)*(istage-2)/2+j)/(Direction*H)
	   CALL WAXPY(5,HC,K(5*(j-1)+1),1,K(ioffset+1),1)
	 END DO
         IF ((.NOT. Autonomous).AND.(ros_Gamma(istage).NE.ZERO)) THEN
           HG = Direction*H*ros_Gamma(istage)
	   CALL WAXPY(5,HG,dFdT,1,K(ioffset+1),1)
         END IF
...
!~~~>  Compute the new solution 
      CALL WCOPY(5,Y,1,Ynew,1)
      DO j=1,ros_S
	 CALL WAXPY(5,ros_M(j),K(5*(j-1)+1),1,Ynew,1)
      END DO

!~~~>  Compute the error estimation 
      CALL WSCAL(5,ZERO,Yerr,1)
      DO j=1,ros_S        
	CALL WAXPY(5,ros_E(j),K(5*(j-1)+1),1,Yerr,1)
      END DO 
...
!~~~> Local variables     
      ...
      CALL WAXPY(5,(-ONE),Fcn0,1,dFdT,1)
      CALL WSCAL(5,(ONE/Delta),dFdT,1)
...
!~~~>    Construct Ghimj = 1/(H*ham) - Jac0
        CALL WCOPY(19,Jac0,1,Ghimj,1)
        CALL WSCAL(19,(-ONE),Ghimj,1)
...
!        CALL  DGETRF( 5, 5, A, 5, Pivot, ising ) 
...
!~~~> Note: for a full matrix use Lapack:
!        NRHS = 1
!        CALL  DGETRS( 'N', 5 , NRHS, A, 5, Pivot, b, 5, INFO )
